...
import type { TrackLaneVisualSegment } from '@/app/lib/daw/rendering/visual-renderer';
...
export const TrackSegmentClip = forwardRef<TrackSegmentClipHandle, TrackSegmentClipProps>(
  function TrackSegmentClip(
    ...
    useEffect(() => {
      ...
      async function init() {
        try {
          ...
          ws.on('error', (error) => {
            console.warn('[TrackSegmentClip] Failed to decode audio waveform', {
              trackVersionId,
              mimeType,
              error,
            });
          ...
          console.warn('[TrackSegmentClip] Failed to initialize waveform', {
            trackVersionId,
            mimeType,
            error,
          });
    ...
    }, [mimeType, onDurationReady, storageKey, trackVersionId]);
    ...
    return (
      ...
      >
        ...
        {loadError ? (
          <div className="pointer-events-none absolute inset-0 z-40 flex items-center justify-center bg-gray-950/70 px-2 text-center text-[10px] leading-tight text-gray-400">
            {loadError}
          </div>
        ) : null}
