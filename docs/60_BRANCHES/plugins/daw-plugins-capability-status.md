# Plugin Feature Capability Status

Branch-scoped record of audio-plugin capabilities in the `git-for-music` DAW.
This maps the implementation to the design in
[daw-plugins-in-this-daw.md](./daw-plugins-in-this-daw.md); read that first for
the "why". Theory lives in
[daw-plugins-technical-overview.md](./daw-plugins-technical-overview.md).

**Scope of v1:** real-time **WAM 2.0** effect inserts on tracks (Model A), with
host-owned, serializable, collaborative parameter state. Remote/async (HARP-style)
processing remains an independent, unimplemented capability described below.

**Ground rules (from `AGENTS.md`):** implement in the repo, add/adjust Jest/Vitest
tests, run the relevant checks until green, then refresh the vault docs. Never
weaken existing tests. Keep edits minimal and upstream.

Status as of 2026-07-14: realtime WAM insert v1 is implemented. Remote/async
render plugins remain outside the shipped browser insert chain. Files are
linked relative to this note.

---

## Phase 0 — Confirm the seams (no code)

- **Implemented:** Confirm the `pluginGraphFactory` hook exists and is wired into
  `DemoDawClient`:
  - [`playback-engine.ts`](../../../src/app/lib/daw/engine/playback-engine.ts)
    (`PlaybackEnginePluginGraphFactory`, `ensureTrackBus`).
  - `new AudioPlaybackEngine({ pluginGraphFactory })` in
    [`DemoDawClient.tsx`](../../../src/app/pages/groups/demo/components/daw/DemoDawClient.tsx)
    (factory passed today).
- **Implemented:** Confirm the plugin catalog path round-trips end to end:
  - `PluginMetadata` in
    [`schema.prisma`](../../../packages/db/prisma/schema.prisma).
  - `DawProjectBootstrapPluginDefinition` + `pluginDefinitions[]` in
    [`command-api.ts`](../../../packages/server/app/lib/daw/protocol/command-api.ts).
  - `pluginDefinitions` store in
    [`daw-local-cache.ts`](../../../src/app/lib/daw/engine/daw-local-cache.ts).
  - Read-only **Plugins** tab in `DemoDawClient`.
- **Implemented:** Decide the initial WAM SDK dependency (host runtime + a couple of reference
  effect plugins) and record the decision in
  [`docs/01_PROTOCOLS/architecture-decision-log.md`](../../01_PROTOCOLS/architecture-decision-log.md).

---

## Phase 1 — Data model: plugin instances on a track

Goal: represent an ordered insert chain per `trackVersionId` in state.

- **Implemented:** Add a `HostedPluginInstanceState` type and `plugins: HostedPluginInstanceState[]`to `DawTrack` in [`local-project-state.ts`](../../../src/app/lib/daw/state/local-project-state.ts). Fields: `instanceId`, `pluginKey`, `version`, `backend: 'wam' | 'remote'`,`position`, `bypassed`, `params: Record<string, number>` (normalized),optional `state?: JsonValue` (opaque, JSON-only), optional `stateBlobKey?`.
- **Implemented:** Mirror the type on the server protocol ([`command-api.ts`](../../../packages/server/app/lib/daw/protocol/command-api.ts)) and in the snapshot/bootstrap shapes so it survives reload.
- **Implemented:** Ensure branch/version copy logic carries the plugin chain with the track (verify in the branch-creation path; add a test that a forked version keeps its plugin chain).
- **Implemented:** Tests: extend [`operation-reducer.test.ts`](../../../src/app/lib/daw/state/operation-reducer.test.ts)  fixtures to include tracks with plugin chains and assert they survive replay.

---

## Phase 2 — Operations: log, broadcast, replay

Goal: plugin actions become first-class, versioned operations.

- **Implemented:** Extend the `DawOperationType` union in [`command-api.ts`](../../../packages/server/app/lib/daw/protocol/command-api.ts) with: `PLUGIN_ADDED`, `PLUGIN_REMOVED`, `PLUGIN_REORDERED`, `PLUGIN_PARAM_SET`, `PLUGIN_BYPASS_SET`, `PLUGIN_STATE_SET`.
- **Implemented:** Define matching payload interfaces (each carries `trackVersionId` + `instanceId`; param op carries `paramId` + normalized `value`).
- **Implemented:** Add reducer cases in [`operation-reducer.ts`](../../../src/app/lib/daw/state/operation-reducer.ts) for each op (add/remove/reorder/param/bypass/state), keeping the chain ordered by `position`.
- **Implemented:** Mirror the same operation types in the server
  [`snapshot-builder.ts`](../../../packages/server/app/lib/daw/server/snapshot-builder.ts)
  `DemoDawOperationType` union and command-API classification sets in
  [`command-api.ts`](../../../packages/server/app/lib/daw/server/command-api.ts):
  - Add them where timeline-edit / broadcast / auto-version behavior is
    decided (decide: do plugin edits advance head, branch, or auto-version?).
    Default: treat like ordinary edits (broadcast, no forced branch), but
    `PLUGIN_ADDED`/`PLUGIN_REMOVED` may qualify for auto-versioning like
    `TRACK_ADDED`/`TRACK_REMOVED`.
- **Implemented:** Client emit helpers: add methods on the sync/command layer used by
  `DemoDawClient` to emit each op (follow the existing pattern used for
  gain/pan/fade edits).
- **Implemented:** Tests:
  - Reducer unit tests for every new op (add, param, reorder, remove,
    bypass, state, replay determinism).
  - Server command/classification tests (broadcast + snapshot inclusion +
    auto-version decision).

---

## Phase 3 — Real-time WAM host adapter (browser)

Goal: turn a `HostedPluginInstanceState` into a live `AudioNode` chain.

- **Implemented:** Implement a `PlaybackEnginePluginGraphFactory` that, for a `trackVersionId`:
  - reads the ordered insert chain from current project state,
  - builds `input -> wam1 -> wam2 -> ... -> return`,
  - returns the tail node (or `input` if the chain is empty or all bypassed),
  - respects `bypassed` (route around the node).
- **Implemented:** Guard rails from the overview Section 2.1 / 3.3:
  - Instantiate/configure nodes on the main thread only — never in the render
    callback.
  - Pre-resolve module imports before building the graph.
- **Implemented:** Tests: unit-test the factory's chain-building with a mocked WAM node
  (ordering, bypass, empty chain returns input unchanged).

---

## Phase 4 — Wire the engine + live updates

Goal: connect the adapter to the running engine and keep it in sync.

- **Implemented:** Pass the factory when constructing the engine in
  [`DemoDawClient.tsx`](../../../src/app/pages/groups/demo/components/daw/DemoDawClient.tsx):
  `new AudioPlaybackEngine({ pluginGraphFactory })`.
- **Implemented:** Add engine methods to react to chain changes without a full restart where
  possible:
  - `rebuildTrackPluginChain(trackVersionId)` — tear down + rebuild that
    track's bus (chain add/remove/reorder).
  - `setPluginParam(trackVersionId, instanceId, paramId, value)` — live knob
    update via `WamNode.setParameterValues` (no graph rebuild).
  - `setPluginBypass(...)`.
- **Implemented:** Ensure `setProject` diffs plugin chains and applies the minimal update
  (param-only changes must not glitch playback by rebuilding the graph).
- **Implemented:** Handle disposal: disconnect/`destroy` WAM nodes in `dispose()` and when a
  track bus is removed.
- **Implemented:** Tests: engine tests asserting param updates don't rebuild the graph, and
  chain edits do.

---

## Phase 5 — UI: from read-only list to a working insert chain

Goal: let users add, order, tweak, and bypass plugins.

- **Implemented:** Extend the existing **Plugins** browser tab in `DemoDawClient` so a listed
  plugin can be **added to the selected track** (button or drag), emitting
  `PLUGIN_ADDED`.
- **Implemented:** Add a per-track **insert-chain panel** (rack) showing ordered instances with:
  - reorder (drag) -> `PLUGIN_REORDERED`,
  - remove -> `PLUGIN_REMOVED`,
  - bypass toggle -> `PLUGIN_BYPASS_SET`.
- **Implemented:** Render a **generic parameter editor** driven by `parameterSchema`
  (sliders/toggles from id/label/range/unit/default) that emits `PLUGIN_PARAM_SET`
  on change. (Custom per-plugin GUIs are out of scope for v1 — overview Section
  3.3 point 4: UI and DSP are separable, so a generic editor is sufficient first.)
- **Implemented:** Reflect remote collaborators' plugin edits live (they arrive as broadcast
  operations and flow through the reducer -> engine).
- **Implemented:** Tests: extend
  [`DemoDawClient.interaction.test.tsx`](../../../src/app/pages/groups/demo/components/daw/DemoDawClient.interaction.test.tsx)
  to add a plugin, change a param, reorder, remove, and assert the emitted ops.

---

## Remote and Async Processing Boundary

Heavy or AI processors are designed as versioned server renders (Model B), but
this path is not implemented.

- **Not implemented:** A remote job type in the processing pipeline
  ([processing jobs](../../architecture/processing-jobs.md)) carrying `pluginKey`,
  `version`, params, `inputStorageKey`, `outputStoragePrefix`.
- **Not implemented:** A server runner that writes derived output to a new
  prefix, preserves the original, and commits a new track version through the
  derived-version flow. A local worker is the nearest deployment model;
  SQS/Fargate remains a possible later backend.
- **Not implemented:** Realtime announcement of the derived version to viewers.
- **Not implemented:** A "Render with..." UI using `parameterSchema` and the
  existing processing-job status surface.
- **Not implemented:** End-to-end coverage proving enqueue, derived-version
  creation, and original-audio preservation.

---

## Phase 7 — State durability, latency, and hardening

- **Implemented:** Verify a full **reload** reconstructs plugin chains + params from the
  operation tail + latest snapshot (add an integration/replay test).
- **Implemented:** Enforce **JSON-only** plugin state in the op log; route large binary state
  through an asset/blob key (`stateBlobKey`) instead of inlining bytes.
- **Implemented:** Latency: capture WAM-reported latency per instance; document that the engine
  does **not** yet do plugin-latency compensation and track it as a known gap.
- **Implemented:** Failure handling: a plugin that fails to load must not break playback —
  fall back to `input` passthrough and surface a non-blocking error.
- **Implemented:** Performance: cap concurrent WAM instances / warn on heavy chains.

---

## Verification Evidence

- **Implemented:** `pnpm` Vitest suites touching:
  - [`operation-reducer.test.ts`](../../../src/app/lib/daw/state/operation-reducer.test.ts)
  - [`project-sync-engine.test.ts`](../../../src/app/lib/daw/engine/project-sync-engine.test.ts)
  - [`DemoDawClient.interaction.test.tsx`](../../../src/app/pages/groups/demo/components/daw/DemoDawClient.interaction.test.tsx)
  - `wam-host` / factory tests
- **Implemented:** Server command / snapshot-builder tests for the new operation types.

## Verified V1 Outcomes

- **Implemented:** A user can add a WAM effect to a track, tweak its parameters, reorder,
  bypass, and remove it — and **hear** the effect during playback.
- **Implemented:** Every action is broadcast to collaborators in realtime and applies to their
  playback graph.
- **Implemented:** Plugin chains + parameters survive reload and travel correctly across
  branches/versions.
- **Implemented:** All existing and new tests pass; vault docs (this note + the design note)
  are refreshed to match what shipped.
